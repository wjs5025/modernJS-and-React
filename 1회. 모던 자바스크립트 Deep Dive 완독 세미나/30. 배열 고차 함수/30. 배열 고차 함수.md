...
## Array.prototype.map<U>(acllbackfn : (value:T, index:number, array:T[]) => U, thisArg?:any) : U[]
...
const result = numbers.filter((item, index, self) => item % 2); // 홀수만 필터링
console.log(result);
```
...
## Array.prototype.reduce<U>(calback:(state:U, element:T, index:number,array:T[]) => U, firstState?:U) : U
...
const sum_solved_correct = student.reduce(
  ...
  0
); // 중요한 점 : prev는 이미 누산된 숫자이고, 객체의 프로퍼티 합산시에는 초기값을 지정(두번째 인수)해주어야 한다.
...
## Array.prototype.soe(callback:(value :T, index : number, array : Array) => boolean, thisArg?:any) : boolean

- 배열 내 **일부요소가 콜백 함수의 테스트를 통과하는지 확인**하고 그 결과를 boolean으로 반환.
- 콜백함수의 매개변수로 요소(item), 인덱스(index), self(this)를 전달받을 수 있다.
...
- 배열 내 **모든 요소가 콜백함수의 테스트를 통과하는지 확인**하여, 그결과를 boolean으로 반환.
- 콜백함수의 매개변수로 요소(item), 인덱스(index), self(this)를 전달받을 수 있다.
...
console.log(result_find); // { id: 1, name: "Jeon" } - Hwang은 나오지 않는다.
