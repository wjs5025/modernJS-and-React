...
- 화살표 함수는 생성자함수로 사용할 수 없다.
- 생성자함수는 prototype 프로퍼티를 가지며, prototype 프로퍼티가 가리키는 프로토타입 객체의 constructor를 사용한다.
...
- addEventListener()의 **콜백함수를 일반 함수로 정의**하면, this는 이벤트를 호출한 주체(바인딩된 요소, currentTarget)를 가리킨다.
- addEventListener()의 **콜백함수를 화살표 함수로 정의**하면, this는 상위 컨텍스트인 전역 객체 window를 가리킨다.
- 따라서, addEventListener()의 콜백함수를 정의할 때에는, 일반 함수(function 키워드)를 사용하는 것이 좋다.
